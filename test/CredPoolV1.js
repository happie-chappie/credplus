...
describe("CredPoolV1 contract", function () {
  let CredPoolV1;
  ...
  describe("Borrower borrowing transactions", function () {
    before(async () => {
      // borrowing from the CredPoolV1
      await hardhatCredPoolV1.connect(borrower).borrow(BORROWING_DAI);
    });
  ...
  describe("Borrower repaying transactions", function () {
    before(async () => {
      // we approve the CredPoolV1 to move DAI funds from borrower
      await DAI.connect(borrower).approve(credPoolAddress, BORROWING_DAI);
      await hardhatCredPoolV1.connect(borrower).repay(BORROWING_DAI);
    });
  ...
  */
  describe("Lender depositing transactions", function () {
    before(async () => {
      ...
      await hardhatCredPoolV1.connect(lender).deposit(LENDING_DAI);
    });
  ...
  describe("Lender withdrawing transactions", function () {
    before(async () => {
      await hardhatCredPoolV1.connect(lender).withdraw(LENDING_DAI);
    });
